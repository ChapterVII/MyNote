...
- 产品上对于健壮性和稳定性的顾虑：由于Node执行再单线程上，一旦单线程上抛出的异常没有被捕获，将会引起整个进程的崩溃。

### 服务模型的变迁

1. 石器时代：同步

   最早的服务器的执行模型是同步的，服务模式是一次只为一个请求服务，所有请求都得按次序等待服务。处理能力相当低下，已基本被淘汰，只在一些无并发要求的应用中存在。

2. 青铜时代：复制进程

   通过进程的复制同时服务更多的请求和用户。每个连接都需要一个进程来服务，即100个连接需要启动100个进程来进行服务，是非常昂贵的代价。复制的过程中，需要复制进程内部的状态，对于每个连接都进行这样的复制的话，相同的状态将会在内存中存在很多份，造成浪费，并且由于要复制较多数据，启动也较为缓慢。

   预复制：解决启动缓慢的问题，即预先复制一定数量的进程。同时将进程复用，避免进程创建，销毁带来的开下。但不具备伸缩性，一旦并发请求过高，内存使用随着进程数的增长将会被耗尽。

3. 白银时代：多线程

   解决进程复制中的浪费问题，让一个线程服务一个请求。线程相对进程的开销要小很多，并且线程之间可以共享数据，解决内存浪费的问题，并利用线程池可以减少创建和销毁线程的开销。但面临的并发问题只能说比多进程略好，因为每个线程都有自己独立的堆栈，都需要占用一定的内存空间。另外，一个CPU核心在一个时刻只能做一件事情，操作系统只能通过将CPU切分为时间片的方法，让线程可以较为均匀的使用CPU资源，但是操作系统内核在切换线程的同时也要切换线程的上下文，当线程数量过多时，时间将会被耗用在上下文切换中。所以在大并发量时，多线程结构还是无法做到强大的伸缩性。

4. 黄金使得：事件驱动

   解决高并发问题，采用单线程避免了不必要的内存开销和上下文切换开销。

   由于所有处理都是在单线程上进行，影响事件驱动服务模型性能的点在于CPU的计算能力，它的上限决定这类服务模型的性能上限，但不受多进程或多线程模式中资源上限的影响，可伸缩性远比前两者高。如果解决掉多核CPU的利用问题，带来的性能上提升是可观的。

### 多进程架构

面对单进程单线程对多核使用不足的问题，经验是启动多进程即可。理想状态下每个进程各自利用一个CPU，以此实现多核CPU的利用。Node提供了child_process模块，也提供了child_process.fork()函数实现进程的复制。

Master-Worker模式：主从模式。主进程不负责具体的业务处理，而是负责调度和管理工作进程，是趋向于稳定的。工作进程负责具体的业务处理，其稳定性值得开发者关注。

![Master-Worder模式](https://github.com/GrowLegend/MyNote/blob/master/static/images/深入浅出Nodejs/Master-Worder模式.png)

通过fork()复制的进程都是一个独立的进程，这个进程中有着独立而全新的V8实例。需要至少30毫秒的启动时间和至少10MB的内存。fork()进程是昂贵的。

1. 创建子进程：4个方法可用于创建子进程，且创建子进程后均会返回子进程对象。后三种都是spawn()的延伸应用。

   ```javascript
   const cp = require('child_process');
   cp.spawn('node', ['worker.js']);
   // exec&execFile:创建时可以指定timeout属性设置超时时间，一旦创建的进程运行超过设定的时间将会被杀死
   // exec:适合执行已有的命令
   ...
   可执行文件是指可以直接执行的文件，如果JavaScript文件通过execFile()运行，它的首行内容必须添加#!/usr/bin/env node

2. 进程间通信

   子进程对象由send()方法实现主进程向子进程发送数据，message事件实现收听子进程发来的数据。通过消息传递内容，而不是共享或直接操作相关资源，是较为轻量和无依赖的做法。

   为了实现父子进程之间的通信，父进程与子进程之间将会创建IPC通道。通过IPC通道，父子进程之间才能通过message和send传递消息。

   - 进程间通信目的：让不同的进程能够互相访问资源并进行协调工作。

   - 实现进程间通信的技术：命名管道、匿名管道、socket、信号量、共享内存、消息队列、Domain Socket等。

   - 进程间通信原理：IPC全称Inter-Process Communication。Node中实现IPC通道的是管道（pipe）技术。在Node中管道是个抽象层面的称呼，具体细节实现由libuv提供，在Windows下由命名管道（named pipe）实现，*nix系统则采用Unix Domain Socket实现。

     父进程在实际创建子进程前，会创建IPC通道并监听它，然后才真正创建出子进程，并通过环境变量（NODE_CHANNEL_FD）告诉子进程这个IPC通道的文件描述符。子进程在启动的过程中，根据文件描述符去连接这个已存在的IPC通道。

     IPC通道使用命名管道或Domain Socket创建的，它们与网络socket行为比较类似，属于双向通信。不同的是它们在系统内核中就完成了进程间的通信，而不用经过实际的网络层，非常高效。Node中，IPC通道被抽象为Stream对象，在调用send时发送数据，接收到的消息会通过message事件触发给应用层。

     注：只有启动的子进程是Node进程时，子进程才会根据环境变量去连接IPC通道，对于其他类型的子进程则无法实现进程间通信，除非其他进程也按约定去连接这个已经创建好的IPC通道。

3. 句柄传递

   如果让多个服务都监听一个相同的端口，那么只有一个工作进程能够监听到该端口，其余的进程在监听的过程中都跑出了EADDRINUS异常，这是端口被占用的情况，新的进程不能继续监听该端口了。解决：让每个进程监听不同的端口，其中主进程监听主端口（如80），主进程对外接收所有的网络请求，再将这些请求分别代理到不同的端口的进程上。

   通过代理，可以避免端口不能重复监听的问题，甚至可以在代理进程上做适当的负载均衡。由于进程每接收到一个连接，将会用掉一个文件描述符，因此代理方案中客户端连接到代理进程，代理进程连接到工作进程的过程需要用掉两个文件描述符。操作系统的文件描述符是有限的，代理方案浪费掉一倍数量的文件描述符的做法影响了系统的扩展能力。

   Node引入了进程间发送句柄的功能来解决上述问题。send方法处理能通过IPC发送数据外，还能发送句柄。
   ...
   句柄：是一种可以用来标识资源的引用，内部包含了指向对象的文件描述符。可以用来标识一个服务器端socket对象、一个客户端socket对象、一个UDP套接字、一个管道等。

   前一个问题中，可以去掉代理的方案，使主进程接收到socket请求后，将这个socket直接发送给工作进程，而不是重新与工作进程之间建立新的socket连接来转发数据。
   ...
   - 句柄发送与还原
     ...
     - net.Socket：TCP套接字
     - net.Server：TCP服务器，任意建立在TCP服务上的应用层服务都可以
     - dgram.Socket：UDP套接字
     - dgram.Native：C++层面的UDP套接字

     send()方法将消息发送到ICP管道前，将消息组装成两个对象，一个参数使handle，另一个是message。
     ...
     发送到IPC管道中的实际上句柄文件描述符，文件描述符实际上是一个整数值。message对象在写入IPC管道时会通过JSON.stringify()进行序列化。最终发送到IPC管道中的信息都是字符串，send()方法能发送消息和句柄并不意味着它能发送任意对象。

     连接了IPC通道的子进程可以读取到父进程发来的消息，将字符串通过JSON.parse()解析还原为对象后，才出发message事件将消息体传递给应用层使用。这个过程中，消息对象还要被过滤处理，message.cmd值如果以NODE_为前缀，将响应一个内部事件InternalMessage。如果message.cmd值时NODE_HANDLE，将去除message.type值和得到的文件描述符一起还原出一个对应的对象。

     由于底层细节不被应用层感知，在子进程中，开发者会有一种服务器就是从父进程中直接传递过来的错觉。注意：Node进程之间只有消息传递，不会真正地传递对象，这种错觉是抽象封装地结果。

     目前Node只支持上述提到地几种句柄，并非任意类型的句柄都能在进程之间传递，除非由完整的发送和还原的过程。

   - 端口共同监听：为何通过发送句柄后，多个进程可以监听到相同的端口而不引起EADDRINUSE异常？

     独立启动的进程中，TCP服务器端socket套接字的文件描述符并不相同，导致监听到相同的端口时会抛出异常。

     Node底层对每个端口监听都设置了SO_REUSEADDR选项，涵义是不同进程可以就相同的网卡和端口进行监听，这个服务器端套接字可以被不同的进程复用。

     由于独立启动的进程互相之间并不知道文件描述符，所以失败。但对于send()发送的句柄还原出来的服务而言，它们的文件描述符相同，所以不会异常。

     多个应用监听相同端口时，文件描述符同一时间只能被某个进程所用。网络请求向服务器端发送时，只有一个进程能抢到连接，只有它能为这个请求进行服务。这些进程服务是抢占式的。

### 集群稳定之路

1. 进程事件

   父进程能监听到的与子进程相关的事件，除了message外还有：

   - error：子进程无法被复制创建、无法被杀死、无法发送消息时会触发。
   - exit：子进程退出时触发。正常退出的第一个参数为退出码，否则为努力了。如果进程是通过kill()方法杀死的，会得到第二个参数，标识杀死进程时的信号。
   - close：子进程的标准输入输出流中止时触发，参数与exit相同。
   - disconnect：父进程或子进程中调用disconnect()方法时触发，调用该方法时将关闭监听IPC通道。

   父进程给子进程发送消息的事件除了send()外，还有kill()。kill()方法并不能真正将通过IPC相连的子进程杀死，只是给子进程发送了一个系统信号。默认发送的是SIGTERM信号。与进程默认的kill()方法类似。

   POSIX标准中有一套完备的信号系统，在命令行中执行kill -l可以看到详细的信号列表。Node提供了这些信号对应的信号事件，每个进程都可以监听这些信号事件。这些信号事件是用来通知进程的，每个信号事件有不同的含义，进程在收到响应信号时，应当作出约定的行为。
...
2. 自动重启
   ...
   // 限量重启：
   // 工作进程不能无限制地被重启，如果启动的过程中发生错误或启动后接到连接就接收到错误，会导致工作进程频繁重启，这种不属于捕捉未知异常的情况
   // 为了消除这种无意义的重启，一定规则限制：如在单位时间内规定只能重启多少次，超过限制就触发giveup事件
   ...
   const createWorker = function() {
     ...
     worker.on('exit', function() {
       ...
       // 主进程在侦听到工作进程的exit后，会立即启动新的进程服务，保证整个集群中总是有进程在为用户服务的。
       // createWorker();
     });
     // 句柄转发
   ...
   // 为了避免隐藏的bug导致工作进程退出
   // 解决：一旦有未捕获的异常出现，工作进程就会立即停止接收新的连接
   process.on('uncaughtException', function(err) {
     // 记录日志，方便定位和追踪代码一场出现的位置
     logger.error(err);
     // 极端情况下，所有工作进程都停止接收新的连接，全处在等待退出的状态。但在等到进程完全退出才重启的过程中，所有新来的请求可能存在没有工作进程为新用户服务的情景，丢掉大部分请求。
     // 解决：在退出的流程中增加一个自杀（suicide）信号。
     ...
     // 等待长连接断开可能需要较久的时间，为此设置一个超时时间，在限定时间里强制退出。
     setTimeout(function() {
       process.exit(1);
     }, 5000);
   });
   ...
   giveup事件比uncaughtException更严重。uncaughtException只代表集群中某个工作进程退出，在整体性保证下，不会出现用户得不到服务的情况。但giveup事件则标识集群中没有任何进程服务。为了健壮性考虑，应在giveup事件中添加重要日志，并让监控系统监视到这个严重错误，进而报警等。

3. 负载均衡

   保证多个处理单元工作量公平的策略。

   Node默认提供的机制是采用操作系统的抢占式策略，就是在一堆工作进程中，闲着的进程对到来的请求进行争抢，谁抢到谁服务。

   对于Node而言，繁忙是由CPU、I/O两个部分构成的，影响抢占的是CPU的繁忙度。对不同的业务，可能存在I/O繁忙，CPU较空闲的情况，这可能造成某个进程能够抢到较多请求，形成负载不均衡的情况。

   Node v0.11中提供了一种新的策略：Round-Robin，又叫轮叫调度。工作方式是由主进程接受连接，将其以此分发给工作进程。也可以通过代理服务器来实现，但是会导致服务器上消耗的文件描述符是平常方式的两倍。

4. 状态共享

   - 第三方数据存储：解决数据共享的最直接简单的方式，如将数据存放到数据库、磁盘文件、缓存服务中，所有巩固走进程启动时将其读取进内存中。
     - 问题：如果数据发生改变，还需要一种机制通知到各个子进程，使内部状态得到更新。
     - 实现状态同步的机制：各个子进程去向第三方进行定时轮询，但问题是轮询时间不能过密，如果子进程过多，会形成并发处理，如果数据没变，会没有意义，白白增加查询装填的开销。如果轮询时间过长，数据改变时不能及时更新到子进程中，有延迟。
   - 主动通知：当数据发生更新时，主动通知子进程。需要一种机制来及时获取数据的改变。仍然不能脱离轮询，但可以减少轮询的进程数量。将用来发送通知和查询状态是否更改的进程叫做通知进程。
...
意义：在用户消费产出的代码之前，开发者首先消费它，给予其重要的保证。
...
2. 介绍
   ...
   - 断言：用于检查程序在运行时是否满足期望。assert模块。

   - 测试框架：用于为测试服务，并不参与测试，主要用于管理测试用例和生成测试报告，提升测试用例的开发速度，提高测试用例的可维护性和可读性，以及一些周边性的工作。如mocha。

     - 测试风格：
       - TDD（测试驱动开发）：关注所有功能是否被正确实现，每个功能都具备对应的测试用例。表述方式偏向于功能说明书的风格。对测试用例的组织主要采用suite和test完成。suite也可以实现多层级描述，测试用例用test。钩子函数仅包含setup和teardown，对应BDD中的before和after。
       - BDD（行为驱动开发）：关注整体行为是否符合预期，适合自顶向下的设计方式。表述方式接近于自然语言的习惯。对测试用例的组织主要采用describe和it进行组织。describe可以描述多层级的结构，具体到测试用例时，用it。还提供before、after、beforeEach、afterEach这4个钩子方法。before和after分别在进入和退出describe时触发执行，beforeEach和afterEach则分别在describe中每个测试用例it执行前后出发执行。
     - 测试报告：mocha --reports即可查看所有的报告格式，默认格式为doc。

   - 测试代码的文件组织：测试代码存在于test目录中。

   - 测试用例：一个行为或功能需要有完善的、多方面的测试用例，一个测试用例中包含至少一个断言。最少需要通过正向测试和反向测试来保证测试对功能的覆盖。对Node而言，还有：

     - 异步测试：

       ```javascript
       // mocha例子
       // 通过检查it回调函数的形参长度来判断是否为异步调用
       it('fs.readFile should be ok', function(done) {
           fs.readFile('file_path', 'utf-8', function(err, data) {
               should.not.exist(err);
               // 测试代码需主动调用done()函数通知测试框架当前测试用例执行完成，然后测试框架才进行下一个测试用例的执行。
               done();
           });
       });
       ```

     - 超时设置：无法知道done()具体再什么时间执行，如果代码偶然出错，导致done()一直没有执行，会造成所有测试用例处于暂停状态。mocha默认超时时间为2000ms。可以再测试用例it或描述describe中调用this.timeout(ms)特殊设置。

   - 测试覆盖率：

     - jscover模块：编译部分通过Java实现，环境依赖上多出了Java；需要编译代码到一个额外的新目录，这个过程相对麻烦。
     - blanket模块：由纯JavaScript实现，编译代码的过程也是隐式的，无须配置额外的目录，对于原模块项目没有额外的侵入。与jscover的原理基本一致，在实现过程上差别在于blanket将编译的步骤注入在require中，技巧在require中。

   - mock：模拟异常，通过伪造被调用方来测试上层代码的健壮性。正确的 mock方式是尽量让mock后的行为与原始行为保持一致。muk模块。
...
1. 基准测试：统计在多少时间内执行了多少次某个方法，一般以次数为参照物，比较时间。benchmark模块
2. 压力测试：指标：吞吐率、响应时间、并发数，反应了服务器的并发处理能力。常用工具：ab、siege、http_load等。
3. 基准测试驱动开发BDD
4. 测试数据与业务数据的转换
...
1. 目录结构
2. 构建工具
   - Makefile：只在*nix操作系统下有效
   - Grunt：解决了Makefile无法跨平台的问题
3. 编码规范
4. 代码审查
...
2. 部署操作：直接命令行执行node files.js以启动应用的方式存在两个问题：占住一个命令行窗口，且随着窗口的退出会导致打开的进程一并退出。

   nohub和&以不挂断进程的方式执行。

   需要一个脚本来实现应用的启动、停止和重启等操作。
...
1. 访问日志：一般用来记录每个客户端对应用的访问。Web引用中，主要记录请求的关键数据。中间件框架Connect提供了一个日志中间件，可以将关键数据按一定格式输出到日志文件中。
2. 异常日志：用来记录哪些意外产生的异常错误。
3. 日志与数据库：数据库比日志文件好的地方在于是结构化数据，可以直接编写SQL语句进行分析，日志文件则需要再加工之后才能分析。但写日志是轻量的方法，将日志分析和日志记录分离开来较好。
4. 分割日志：大量日志产生的情况，按日期分割。

### 监控报警

- 监控（业务逻辑型/硬件型）：目的是将一些重要指标采样记录下来，一旦指标发生较大变化，可以配合报警系统将问题反馈到负责人。
  1. 日志监控：业务逻辑型的主要体现。
  2. 响应时间
  3. 进程监控
  4. 磁盘监控
...
典型的水平扩展方式就是多进程、多机器、多机房的分布式涉及

- 多机器：利用更多的硬件资源，为更多的请求服务。需要考虑负载均衡、状态共享和数据一致性等问题。
- 多机房：为了解决地理位置给用户访问带来的延迟等问题。
- 容灾备份

### 异构共存

选择将一门新技术应用在生产环境中就要考虑与已有的系统或者服务能否异构共存。
